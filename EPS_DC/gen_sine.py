...
def loadGen(signal):
    waveform = [int((signal[n]/signal.max())*32767/10) for n in range(0, len(signal))]
...
base_freq = 50
freqs = np.arange(450, 1000, 50) # от 50 до 400 с шагом 50
...
for freq in freqs:
    ...
    an_out, bn_out, F_out, FFTfreqs = getFFT(voltage)     #
    ######################################
    thd_out = calculate_thd(F_out, freq, base_freq)
    res.append({
        "freq": freq, 
        "coefA_out": an_out, 
        "coefB_out": bn_out,
    ...
    plt.plot(FFTfreqs[:40], np.abs(F_out[:40])*2/N)
    plt.savefig(f'harmonics@{freq}Hz.jpg', dpi = 600)
    plt.close()    
